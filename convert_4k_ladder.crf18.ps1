...
function Invoke-Crf18LanczosControl {
    ...
    Write-Host "24 fps only (same rate as VSR-Eval)."
    ...
    foreach ($file in $files) {
        ...
        Write-Host ""
        Write-Host "[$index/$($files.Count)] $($file.Name)  (source $([Math]::Round($srcFps, 3)) fps -> 24 fps)" -ForegroundColor Cyan

        foreach ($step in $steps) {
            $lrName = Get-Crf18ControlName $name $step.Name $fpsSuffix
            $upName = Get-Lanczos4kControlName $name $step.Name $fpsSuffix
            ...
            $vfDown = Get-VideoFilter $step.Width $step.Height $fps
            $ffArgs = @(
                "-y",
                "-hide_banner"
            ) + $rawArgs + @(
            ...
            Write-Host "CRF18 $($step.Name)$fpsSuffix + Lanczos 4K"
            & $ffmpeg @ffArgs
            $code = $LASTEXITCODE

...
function Invoke-EncodeLadder {
    ...
    foreach ($file in $files) {
        ...
        if ($jobs.Count -eq 1) {
            $j = $jobs[0]
            $ffArgs += @(
                "-vf", ("scale={0}:{1}:flags=lanczos" -f $j.Width, $j.Height),
                "-map", "0:v:0", "-map", "0:a?",
                "-c:v", "libx265", "-crf", $j.Crf, "-preset", "medium",
            ...
            for ($i = 0; $i -lt $jobs.Count; $i++) {
                $labels += "s$i"
                $filters += ("[s{0}]scale={1}:{2}:flags=lanczos[v{0}]" -f $i, $jobs[$i].Width, $jobs[$i].Height)
            }
